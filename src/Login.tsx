import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from './contexts/AuthContext';
import { supabase } from './lib/supabaseClient';
import './Login.css';

function Login() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [provider, setProvider] = useState(null); // null, 'email', 'google', or 'false'
  const [checkingProvider, setCheckingProvider] = useState(false);
  const { signInWithEmail, signInWithGoogle, user } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    // If already logged in, redirect to profile
    if (user) {
      navigate('/profile');
    }
  }, [user, navigate]);

  const checkEmailProvider = async (e) => {
    e.preventDefault();
    setError('');
    setCheckingProvider(true);

    try {
      // Call the Supabase function to check provider
      const { data, error } = await supabase
        .rpc('get_provider_by_email', { p_email: email });

      if (error) {
        console.error('Error checking provider:', error);
        setError('Hiba történt. Kérjük, próbálja újra.');
        setCheckingProvider(false);
        return;
      }

      if (data === 'false') {
        // No user found with this email
        setError('Nincs regisztrált fiók ezzel az email címmel.');
        setCheckingProvider(false);
        return;
      }

      // Set the provider
      setProvider(data);
    } catch (err) {
      console.error('Error:', err);
      setError('Hiba történt. Kérjük, próbálja újra.');
    } finally {
      setCheckingProvider(false);
    }
  };

  const handleEmailLogin = async (e) => {
    e.preventDefault();
    setError('');
    setLoading(true);

    try {
      const { error } = await signInWithEmail(email, password);
      if (error) {
        setError(error.message);
      } else {
        navigate('/profile');
      }
    } catch (err) {
      setError('Hiba történt a bejelentkezés során.');
    } finally {
      setLoading(false);
    }
  };

  const handleGoogleLogin = async () => {
    setError('');
    setLoading(true);

    try {
      const { error } = await signInWithGoogle();
      if (error) {
        setError(error.message);
        setLoading(false);
      }
      // Don't set loading to false here - the redirect will happen
    } catch (err) {
      setError('Hiba történt a bejelentkezés során.');
      setLoading(false);
    }
  };

  const handleChangeEmail = () => {
    setProvider(null);
    setPassword('');
    setError('');
  };

  return (
    <div className="login-page">
      <div className="login-container">
        <div className="login-header">
          <h1>Bejelentkezés</h1>
          <p>Lépj be a ParkSafe fiókodba</p>
        </div>

        {error && (
          <div className="error-message">
            <svg className="error-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <span className="error-text">{error}</span>
          </div>
        )}

        {/* Step 1: Email Input - Show when provider is not determined */}
        {provider === null && (
          <form onSubmit={checkEmailProvider} className="login-form">
            <div className="form-group">
              <label htmlFor="email">Email cím</label>
              <input
                type="email"
                id="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="pelda@email.com"
                required
                disabled={checkingProvider}
              />
            </div>

            <button
              type="submit"
              className="login-button"
              disabled={checkingProvider}
            >
              {checkingProvider ? 'Ellenőrzés...' : 'Tovább'}
            </button>
          </form>
        )}

        {/* Step 2a: Password Input - Show when provider is 'email' */}
        {provider === 'email' && (
          <>
            <div className="email-display">
              <span className="email-text">{email}</span>
              <button 
                type="button" 
                onClick={handleChangeEmail} 
                className="change-email-button"
                disabled={loading}
              >
                Módosítás
              </button>
            </div>

            <form onSubmit={handleEmailLogin} className="login-form">
              <div className="form-group">
                <label htmlFor="password">Jelszó</label>
                <input
                  type="password"
                  id="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="••••••••"
                  required
                  disabled={loading}
                  autoFocus
                />
              </div>

              <button
                type="submit"
                className="login-button"
                disabled={loading}
              >
                {loading ? 'Bejelentkezés...' : 'Bejelentkezés'}
              </button>
            </form>
          </>
        )}

        {/* Step 2b: Google Sign In - Show when provider is 'google' */}
        {provider === 'google' && (
          <>
            <div className="email-display">
              <span className="email-text">{email}</span>
              <button 
                type="button" 
                onClick={handleChangeEmail} 
                className="change-email-button"
                disabled={loading}
              >
                Módosítás
              </button>
            </div>

            <button
              onClick={handleGoogleLogin}
              className="google-button"
              disabled={loading}
            >
              <svg className="google-icon" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              {loading ? 'Bejelentkezés...' : 'Google-lal folytatás'}
            </button>
          </>
        )}
      </div>
    </div>
  );
}

export default Login;
